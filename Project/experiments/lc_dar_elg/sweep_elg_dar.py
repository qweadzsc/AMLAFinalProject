"""Grid search DAR hyperparameters on top of an ELG-lite checkpoint."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Sweep DAR on top of an ELG-lite checkpoint.")
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--dar-log-nearest", type=int, choices=[0, 1], default=1)
    parser.add_argument("--sweep-k", default="5,10,20")
    parser.add_argument("--sweep-alpha", default="0.05,0.1,0.25,0.5,1.0,2.0,4.0")
...
def main():
    ...
    for k, alpha in tqdm(grid, desc="ELG + DAR sweep", unit="run"):
        run_name = f"k{k}_a{str(alpha).replace('.', 'p')}"
        run_dir = args.results_dir / run_name
        print(f"[sweep_elg_dar] start run={run_name} checkpoint={args.checkpoint.name}", flush=True)
        cmd = [
            sys.executable,
            str(THIS_DIR / "eval_all_lc_elg.py"),
            "--checkpoint",
            str(args.checkpoint),
            "--device",
            args.device,
        ...
        print(f"[sweep_elg_dar] finished run={run_name} mean_gap={mean_gap:.2f}%", flush=True)
    ...
    best_path.write_text(json.dumps({"best_by_dataset": best_by_dataset, "best_mean": best_mean}, indent=2) + "\n")

    print(f"[sweep_elg_dar] wrote sweep summary to {summary_path}", flush=True)
    print(f"[sweep_elg_dar] wrote comparison table to {comparison_path}", flush=True)
