...
class LocalPolicyScorer(nn.Module):
    """A lightweight TSP local policy scorer for ELG-style experiments.

    Inputs:
      - coords: (batch, node, 2)
      - current_node: (batch, pomo)
      - visited_mask: (batch, pomo, node), True means unavailable

    Output:
      - local_score: (batch, pomo, node)

    Only the K nearest unvisited neighbors receive learned scores. All other
    nodes are filled with `non_neighbor_value`, which defaults to 0 so the
    local score can be added to a global score without hard-masking nodes.
    """
    ...
    def _pairwise_features(
        ...
        current_coords = self._current_coords(coords, current_node)  # (batch, pomo, 2)
    ...
    def forward(
        ...
        neighbor_idx, neighbor_mask = self._select_local_neighbors(dist, visited_mask)
        ...
        rank_embed = self.rank_embedding(clipped_rank_positions)  # (k, hidden)
