...
## Close 总结：Leader Reward 路线失败原因

本路线可以暂时 close。当前实验说明 Leader Reward 的实现方向有正向信号，但不足以作为后续主线继续投入：

- **提升幅度过小**：长训练后 baseline 与 Leader Reward 的差距仍然很小。80 epoch、50 batches/epoch、batch size 64、同 seed、GPU 5 单卡条件下，Leader Reward 相比 baseline 的 gap 改善分别为 TSP50 uniform `-0.22%`、TSP50 OOD `-1.72%`、TSP100 uniform `-0.58%`。这更像是轻微正则或训练噪声级别的收益，而不是稳定、显著的算法改进。
- **与论文量级不匹配**：Leader Reward 论文报告的是在 POMO 系列模型上更明显的提升，尤其强调对推理阶段 best-of-POMO 目标的训练对齐。但本项目当前 LC baseline、训练预算、实现细节和评测规模都与论文设置不同，因此只能验证“思想可运行”，不能复现论文级别收益。
- **当前实现是简化版**：本实现只在独立训练脚本中对同一 batch 内 reward 最大的 rollout 做 advantage 放大，没有完整复现论文中的训练流程、超参数搜索、多 seed 统计、以及与原始 POMO 官方实现完全一致的训练管线。
- **baseline 本身收敛后留给目标函数的空间有限**：长训练显示主要收益来自训练轮数增加。TSP50 uniform gap 从短训约 `17%` 降到长训约 `4%-5%`，说明之前的主要问题是训练不足；在 baseline 已接近平台期后，Leader Reward 只能带来很小边际收益。
- **缺少官方代码可对齐**：已搜索 arXiv、CatalyzeX 和若干 NCO/COP 索引仓库，未找到 Leader Reward 官方开源 repo。没有官方实现时，继续投入会变成猜测论文细节和调参，性价比较低。
- **课程项目风险较高**：如果继续沿这条路线，需要补充原始 POMO 对齐、多 seed、更多超参数 sweep 和更长训练，成本明显上升，但当前结果不支持它作为主要创新点。

因此，Leader Reward 更适合在报告中作为一个失败/负结果复现实验呈现：实现可运行，GPU 单卡训练确认有效，长训练后有小幅正向结果，但收益不显著，且与论文结果不构成充分一致的复现。后续如果继续做项目，建议转向更容易解释和验证的方向，例如几何对称增强、数据增强消融、混合规模训练，或直接围绕 baseline 训练充分性与泛化做报告。
